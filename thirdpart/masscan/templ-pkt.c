...
static unsigned char default_tcp_template[] =
    ...
    "\x45"          /* IP type */
    "\x40"          /* Flags: 0x40, Don't fragment*/
    "\x00\x40"      /* total length = 40 bytes */
    ...
    "\xb0"          /* header length */
    "\x02"          /* SYN */
    "\xFF\xFF"        /* window fixed to 1024 */
    "\xFF\xFF"      /* checksum */
    "\x00\x00"      /* urgent pointer */
    "\x02\x04\x05\xb4"  /* added options [mss 1460] */
    "\x01"              /* nop*/
    "\x03\x03\x06"      /* TCP Option - Window scale: 6 (multiply by 64) */
    "\x01\x01"          /* nop nop*/
    "\x08\x0a"
    "\0\0\0\0"          /*options time stamp*/
    "\0\0\0\0"          /**/
    "\x04\x02"          /*TCP Option - SACK permitted*/
    "\x00\x00"          /*TCP Option - End of Option List (EOL)*/
;
...
{
    uint64_t xsum;
    uint32_t timestamp = (uint32_t)pixie_gettime();
    ...
    if (ip_them.version == 4) {
        ...
        px[offset_tcp+32] = (unsigned char)(timestamp >> 24);
        px[offset_tcp+33] = (unsigned char)(timestamp >> 16);
        px[offset_tcp+34] = (unsigned char)(timestamp >>  8);
        px[offset_tcp+35] = (unsigned char)(timestamp >>  0);
...
void destory_TemplateSet(struct TemplateSet*p){
