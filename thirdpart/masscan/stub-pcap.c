...
static void seterr(char* errbuf, const char* msg) {
    ...
    if (length > PCAP_ERRBUF_SIZE - 1) length = PCAP_ERRBUF_SIZE - 1;
...
static int null_PCAP_COMPILE(pcap_t* a, struct bpf_program* b, const char* c, int d,
                             unsigned int e) {
    UNUSEDPARM(a);
    UNUSEDPARM(b);
    UNUSEDPARM(c);
...
static void null_PCAP_FREECODE(struct bpf_program* b) {
    UNUSEDPARM(b);
}

static int null_PCAP_LOOKUPNET(const char* ifname, unsigned int* network, unsigned int* mask,
                               char* err) {
    UNUSEDPARM(ifname);
    UNUSEDPARM(network);
    UNUSEDPARM(mask);
...
static int null_PCAP_SETFILTER(pcap_t* p, struct bpf_program* bpf) {
    UNUSEDPARM(p);
    UNUSEDPARM(bpf);
    return 0;
}
...
int pcap_init(void) {
    ...
    if (hPacket == NULL) hPacket = LoadLibraryA("Packet.dll");
    if (hPacket == NULL) {
        if (pl->is_printing_debug) switch (GetLastError()) {
    ...
    if (hLibpcap == NULL) hLibpcap = LoadLibraryA("wpcap.dll");
    if (hLibpcap == NULL) {
