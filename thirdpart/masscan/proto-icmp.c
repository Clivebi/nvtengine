 ...
 ***************************************************************************/
static int
matches_me(struct HostScanTask *task, ipaddress ip, unsigned port)
{
    return is_myself(&task->nic.src, ip, port) ;
}
 ...
 ***************************************************************************/
void
handle_icmp(struct HostScanTask *task, time_t timestamp,
    ...
    switch (type) {
        ...
        case 129:
            ...
            /*output_report_status(
             ...
             parsed->ip_ttl,
             parsed->mac_src);*/
            //result =  lookup_or_new_host_scan_result(task, ip_them);
            add_port_result(lookup_or_new_host_scan_result(task, ip_them), ICMP_REPLAY, 0);
            if(task->icmp && task->count.icmp == (task->range + task->range_ipv6)){
                task->recv_done = 1;
            }
            ...
            switch (code) {
                ...
                case 3: /* port unreachable */
                    if (length - parsed->transport_offset > 8) {
                        ...
                        if (!matches_me(task, ip_me2, port_me2))
                            return;
                        
                        switch (ip_proto) {
                            case 6:
                                add_port_result(lookup_or_new_host_scan_result(task, ip_them2), UDP_CLOSED, port_them2);
                                break;
                            case 17:
                                add_port_result(lookup_or_new_host_scan_result(task, ip_them2), UDP_CLOSED, port_them2);
                                break;
