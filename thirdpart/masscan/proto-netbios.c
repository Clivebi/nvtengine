 ...
 *****************************************************************************/
static unsigned
handle_nbtstat_rr(struct HostScanTask *task, time_t timestamp, unsigned ttl,
                  ...
                  ipaddress ip_them, unsigned port_them)
{
    ...
    add_port_result(lookup_or_new_host_scan_result(task, ip_them), port_them, UDP_NETBIOS);
 ...
 ***************************************************************************/
unsigned
handle_nbtstat(struct HostScanTask *task, time_t timestamp,
    const unsigned char *px, unsigned length, 
    ...
    uint64_t entropy)
{
...

    {
        ...
        return handle_nbtstat_rr(task, timestamp, parsed->ip_ttl,
                                    px + offset,
                                    length - offset,
                                    ip_them,
                                    port_them);
