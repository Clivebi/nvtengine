...
static const char* adapter_from_index(unsigned index) {
    ...
    if (x != -1) {
        ...
        /* Print the list */
        for (d = alldevs; d; d = PCAP.dev_next(d)) {
            if (index-- == 0) return PCAP.dev_name(d);
        }
        return 0;
    } else {
        return 0;
...
int rawsock_send_packet(struct Adapter* adapter, const unsigned char* packet, unsigned length,
    ...
    if (adapter->ring) {
        ...
        if (err < 0) LOG(1, "pfring:xmit: ERROR %d\n", err);
        return err;
    }
    ...
    /* LIBPCAP */
    if (adapter->pcap) return PCAP.sendpacket(adapter->pcap, packet, length);
 ...
 ***************************************************************************/
void rawsock_send_probe_ipv4(struct HostScanTask* task, struct Adapter* adapter,
                             ipv4address ip_them, unsigned port_them, ipv4address ip_me,
    ...
    ipaddress address;
    address.version = 4;
    address.ipv4 = ip_them;
    template_set_target_ipv4(tmplset, ip_them, port_them, ip_me, port_me, seqno, px, sizeof(px),
                             &packet_length);
    if (task != NULL && is_private_address(address)) {
        macaddress_t mac_them;
        if (lookup_arp_table(task, address, &mac_them)) {
            memcpy(px, mac_them.addr, sizeof(macaddress_t));
        }
    }
    rawsock_send_packet(adapter, px, (unsigned)packet_length, flush);
}

void rawsock_send_probe_ipv6(struct HostScanTask* task, struct Adapter* adapter,
                             ipv6address ip_them, unsigned port_them, ipv6address ip_me,
                             unsigned port_me, unsigned seqno, unsigned flush,
                             struct TemplateSet* tmplset) {
    unsigned char px[2048];
    size_t packet_length;
...
const char* rawsock_win_name(const char* ifname) {
    if (is_numeric_index(ifname)) {
        ...
        if (new_adapter_name) return new_adapter_name;
    }
...
static int is_pfring_dna(const char* name) {
    ...
    if (memcmp(name, "zc:", 3) == 0) return 1;
    if (memcmp(name, "dna", 3) != 0) return 0;
...
struct Adapter* rawsock_init_adapter(const char* adapter_name, unsigned is_pfring,
     ...
     *----------------------------------------------------------------*/
    if (is_pfring &&
        !is_pfring_dna(adapter_name)) { /*First ensure pfring dna adapter is available*/
        fprintf(stderr,
                "No pfring adapter available. Please install pfring or run masscan without the "
                "--pfring option.\n");
        return 0;
    }
...
int rawsock_selftest_if(const char* ifname) {
    ...
    default:
        ...
        /* IPv4 router MAC address */
        {
            ...
            stack_arp_resolve(adapter, ipv4, source_mac, router_ipv4, &router_mac, &shutdown);
        ...
        if (!ipv6address_is_zero(ipv6)) {
            ...
            stack_ndpv6_resolve(adapter, ipv6, source_mac, &shutdown, &router_mac);
