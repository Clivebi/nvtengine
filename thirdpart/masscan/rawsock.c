                        ...
                        struct TemplateSet *tmplset)
{
    ...
    if(task != NULL && is_private_address(address)){
        ...
        if(lookup_arp_table(task, address, &mac_them)){
            struct TemplateSet tmplset2[1]= {0};
            template_packet_init(
                                 tmplset2,
                                 task->nic.source_mac,
                                 mac_them,
                                 task->nic.router_mac_ipv6,
                                 task->payloads.udp,
                                 task->payloads.oproto,
                                 stack_if_datalink(task->nic.adapter),
                                 task->seed);
            template_set_target_ipv4(tmplset2, ip_them, port_them, ip_me, port_me, seqno,
                                     px, sizeof(px), &packet_length);
            destory_TemplateSet(tmplset2);
            is_set = 1;
        }
    }
