 ...
 ***************************************************************************/
unsigned
coap_handle_response(struct HostScanTask *task, time_t timestamp,
            ...
            )
{
    ...
    if (is_valid) {
        ...
        add_port_result(lookup_or_new_host_scan_result(task, ip_them), UDP_COAP, port_them);
        banout_release(banout);
        return 0;
    } else {
        banout_release(banout);
        return default_udp_parse(task, timestamp, px, length, parsed, entropy);
    }
}
