...
void mutex_init(LPCRITICAL_SECTION lpSection, void* b) {
    UNREFERENCED_PARAMETER(b);
    InitializeCriticalSection(lpSection);
}
void mutex_lock(LPCRITICAL_SECTION lpSection) {
    EnterCriticalSection(lpSection);
}
void mutex_unlock(LPCRITICAL_SECTION lpSection) {
    LeaveCriticalSection(lpSection);
}
void mutex_destory(LPCRITICAL_SECTION lpSection) {
...
#define mutex_destory(a) pthread_mutex_destroy
#endif
...
int extend_vfs_dir_entry_array(vfs_dir_entry_array_p array, int add_size) {
    int new_size = 0;
    if (array->allocate_size - array->size > (unsigned int)add_size) {
        return ERROR_SUCCESS;
    }
...
int vfs_get_file_full_path(vfs_handle_p vfs, vfs_dir_entry_p entry, char* full_path,
                           int* path_size) {
    int need_size = 0;
    int addSlip = 0;
    ...
    need_size = (int)strlen(parent->name);
    if (need_size > 0) {
    ...
    need_size += (int)strlen(entry->name);
...
const vfs_dir_entry_p vfs_lookup(vfs_handle_p vfs, const char* full_path) {
    ...
    vfs_dir_entry_p entry = NULL;
    char retail_path[VFS_MAX_PATH] = {0};
...
int vfs_raw_read_at(vfs_handle_p vfs, void* buf, long offset, int size) {
    int error = -1;
    if (size == 0) {
        return 0;
    }
    ...
    do {
        ...
        if (size != fread(buf, 1, size, vfs->host_file)) {
            break;
        }
        error = 0;
    } while (0);
...
#ifdef __USE_ZLIB__
void* vfs_get_file_all_content(vfs_handle_p vfs, vfs_dir_entry_p entry, int* error_p) {
    ...
    do {
        ...
        if (entry->compress_size !=
            vfs_raw_read_at(vfs, raw, entry->offset, entry->compress_size)) {
...
#else
void* vfs_get_file_content(vfs_handle_p vfs, vfs_dir_entry_p entry, int offset, int size,
                           int* read_size, int* error) {
    void* content = NULL;
    ...
    if ((unsigned int)(offset + size) > entry->file_size) {
...
void* vfs_get_file_all_content(vfs_handle_p vfs, vfs_dir_entry_p entry, int* error) {
